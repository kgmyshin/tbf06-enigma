...
そのためまずは⑴の処理にフォーカスして、どういった処理になっているのか見ていきましょう。
ローター単体を取り出し、いろんな入力を試して、次の装置にどんな値を出力するのかを観察していきましょう。
実際には、手元にエニグマの実物があるわけでもないので、シミュレーターなどを使ってみると良いと思います。

=== Aに設定したローターにプラグボードからAを入力

プラグボードからローターに@<code>{A}の入力がきているケースを考えます。
ローターは内部でA~Zまでの文字それぞれがが別の文字へ反映されます。
...
このままでは法則がわからないので、このローターに対していろんな入力をしてみましょう。

//table[plguboard_rotor_table_b][Bに設定したローターにいろんな入力をしてみる]{
...
//table[plguboard_rotor_table_c][Cに設定したローターにいろんな入力をしてみる]{
プラグボードの入力  A B C D E F ...
-------------------------------------------------------------
...
//table[plguboard_rotor_table_b_num][Bに設定したローターにいろんな入力をしてみる]{
...
今までの法則だと、@<code>{C}のは@<code>{A}からの距離は@<code>{2}となります。
...
//table[plguboard_rotor_table_b_num][Cを送信するローター側の設定をいろいろ変えてみる]{
...
ただ、これは今までと全く同じで、処理の流れが逆なだけです。

//image[rotor_rotor_3][Bに設定された次の装置からBが返却される]{
//}
...
@<code>{A}から@<code>{Z}の26個の大文字のアルファベット出ない場合は例外を投げるようにします。

//list[alphabet][アルファベットを定義]{
